    ...
    private void drawGraph(Graphics g) {
        ...
        for (int i = 0; i < numberYDivisions + 1; i++) {
            ...
            if (dataPointsY.size() > 0) {
                ...
                String yLabel = ((int) ((getMaxYValue() * ((i * 1.0) / numberYDivisions)) * 100)) / 100.0 + ""; // Generate
                                                                                                                // y-axis
                                                                                                                // label
        ...
        if (plotType.equals("line")) {
            for (int i = 0; i < dataPointsX.size() - 1; i++) {
                ...
                int y1 = height - padding - labelPadding
                        - (int) ((dataPointsY.get(i) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
                int x2 = (i + 1) * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding
                        + labelPadding;
                int y2 = height - padding - labelPadding - (int) ((dataPointsY.get(i + 1) * 1.0) / getMaxYValue()
